...
def convertir_donnee(ma_chaine):
    # si ma_chaine commence par des guillemets, le type désiré est string de
    # toute façon:
    ...
    try:  # sinon, on essaie d'évaluer le type de ma_chaine
        ...
        # La ou les réponses sont de type littéral. On retourne la liste des
        # éléments saisis sans les espaces.
...
class Solution():
    ...
    def creer_questions(self):
        for dic in self.infos:
            self.__dict__[dic[self.mots[0]]] = Correction(
                dic, self.mots).afficher
...
class Correction():
    ...
    def creer_boutons(self):
        clear_output()
        a_afficher = []
        if self.infos[self.mots[1]] != '' and self.infos[self.mots[1]
        ...
        # on rajoute le fait de pouvoir préparer sans afficher la réponse
        # finale dans le cas où on écrit '#' après le mot clef rep_question
        if self.infos[self.mots[2]
                      ] != '' and self.infos[self.mots[2]][0:2] != '#\n':
    ...
    def reponse(self, *args):
        ...
        # Si la réponse est de type code python, on crée une nouvelle cellule.
        if self.infos['reponse_en_python']:
            self.creer_boutons()
...
class Montre():
    ...
    def creer(self):
        for nom in self.noms:
            ...
            # on cherche l'index du dernier point du nom, celui qui précède
            # l'extension
            pos_point = nom.rfind('.')
            # on cherche à présent la position du dernier slash, qui finit
            # d'indiquer le chemin de l'image
...
class Question_ouverte():
    ...
    def afficher(self):
        ...
        # On construit ensuite le ou les éléments graphique permettant de
        # saisir la réponse :
        ...
        for element in self.infos[self.mots[2]]:
            dic = dict()
            # leg=convertir_html(conv(element[self.mots_clefs_graph['legende']]))
            # # on commente le 06/10
            ...
            if element[self.mots_clefs_graph['type']].lower() in [
                    'text', 'texte', 'input']:
            ...
            if element[self.mots_clefs_graph['type']].lower() in [
                ...
                if element[self.mots_clefs_graph['orientation_curseur']].lower(
                ) in ['v', 'vertical', 'verticale', 'colonne', 'column']:
                    orient_curseur = 'vertical'

                # S'agit-il d'un intslider ou floatslider?
                if int(m) == m and int(M) == M and int(pas) == pas:
                    dic['widget'] = widgets.IntSlider(
                        value=valeur, min=m, max=M, step=pas, orientation=orient_curseur)
                else:
                    dic['widget'] = widgets.FloatSlider(
                        value=valeur, min=m, max=M, step=pas, orientation=orient_curseur)

            if element[self.mots_clefs_graph['type']].lower(
            ) in ['menu', 'list', 'liste', 'deroulant']:
                dic['widget'] = widgets.Dropdown(
                    options=eval(element[self.mots_clefs_graph['contenu']]), description='')
            orientation = 'row'
            alignement = 'center'

            if element[self.mots_clefs_graph['orientation']].lower(
            ) in ['v', 'vertical', 'verticale', 'colonne', 'column']:
                orientation = 'column'
                alignement = 'flex-start'
            # création du conteneur, avec disposition horizontale ou verticale
            # de legende et saisie
    ...
    def corriger(self, *args):
        ...
        complet = not (
            '' in [
                dic['widget'].value for dic in self.zones_reponses])
...
class Ouvertes():
    ...
    def creer_questions(self):
        for dic in self.infos:
            self.__dict__[dic[self.mots[0]]] = Question_ouverte(
                dic, self.mots_clefs_graph).afficher


# A ce stade, le fichier file doit déjà avoir été crée via preparation.py
# et les différents fichiers text
...
if 'correction' in dic_infos.keys():
    solution = Solution(
        dic_infos['correction'],
        dic_infos['mots_clefs_correction'])
...
if 'ouvertes' in dic_infos.keys():
    question = Ouvertes(
        dic_infos['ouvertes'],
        dic_infos['mots_clefs_ouvertes'],
        dic_infos['mots_clefs_graph'])
